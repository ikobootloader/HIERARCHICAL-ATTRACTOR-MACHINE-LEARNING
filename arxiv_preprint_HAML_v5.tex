...
dissipative dynamical system toward stable basins of attraction across multiple
hierarchical levels. Each level operates in a PCA-decomposed subspace of decreasing
dimension; levels are coupled through bottom-up (error-prediction) and top-down
(context-prediction) feedback flows inspired by Predictive Coding.

\textbf{Primary contributions.}
The paper makes two contributions we consider robust, and several theoretical
observations that we present with explicit caveats:
\begin{itemize}
  \item \textbf{Architectural} (Sections~\ref{sec:architecture}--\ref{sec:learning}): a
        specific assembly of hierarchical PCA subspaces, gradient-learned continuous
        Gaussian attractors, and bidirectional PC-style coupling, trained end-to-end
        with a three-phase progressive schedule. The combination is original
        (Section~\ref{sec:related}) even though each individual primitive has direct
        antecedents.
  \item \textbf{Practical (Condition I1)} (Section~\ref{sec:i1}): a dimension-scaled
        kernel initialisation $\sigma_{\mathrm{init}}^{(l)} = \sqrt{d_l}\cdot\sigma_{\mathrm{data}}^{(l)}$
        that prevents Gaussian-kernel saturation in high dimension and is empirically
        \emph{necessary} for the framework to train beyond $\sim$20 dimensions
        (Table~\ref{tab:kernelfix}). Without~I1, gradient magnitudes collapse to zero
        and learning halts.
\end{itemize}

\textbf{Theoretical observations (with caveats).}
We present four theoretical observations that, on close inspection, are either
specialisations of standard results or carry significant qualifications. We state these
caveats upfront because they affect how the results should be read:
\begin{itemize}
  \item Proposition~\ref{prop:convergence} (dissipative convergence via LaSalle) is a
        standard property of any damped gradient flow with a bounded-below energy; in
        HAML, it justifies the ODE stopping criterion rather than constituting a deep
        result.
  \item Proposition~\ref{prop:dimindep} (dimension-independent local rate at fixed
        points) is mathematically correct but stands in \emph{tension} with
        Condition~I1, which itself scales $\sigma^2 \propto d_l$. The two together
        imply that the local rate scales as $1/d_l$ in practice; the result describes
        a property of the linearised regime, not a global dimension-independence
        guarantee.
  \item Proposition~\ref{prop:expressivity} (exponential expressivity gain in
        attractor count) is a worst-case representational statement; our experiments
        all use $L \in \{2, 3\}$, so the exponential factor is not empirically
        exercised.
  \item Theorem~\ref{thm:map} (MAP equivalence with hierarchical PC under M1--M3)
        requires $\lambda = 0$, i.e.\ \emph{disabling the repulsive terms} that are
        otherwise central to HAML. In the operational regime ($\lambda > 0$), the
        energy decomposes as $\mathcal{E} = \mathcal{F}_{\mathrm{PC}} + \lambda\,\mathcal{F}_{\mathrm{discrim}}$,
        which is a PC--inference / contrastive-discriminative hybrid. The exact MAP
        correspondence applies only to a reduced version of the model.
\end{itemize}
The principled lesson is that HAML's theoretical scaffolding is a coherent
\emph{specialisation} of established frameworks (LaSalle stability, neural-ODE
generalisation theory, PC--inference correspondences), not a foundational
contribution. We restate this explicitly in Section~\ref{sec:theory}.

\textbf{Empirical results (honest summary).}
Six campaigns across synthetic and real datasets. We summarise each with its actual
statistical status:
\begin{itemize}
  \item On a MNIST subset (1500 train / 500 test, 5 epochs), gradient training raises
        accuracy from $45.6\%$ (k-means-only init) to $75.8\%$. \textbf{Bidirectional
        coupling provides no measurable gain} ($75.8\%$ coupled vs.\ $75.8\%$
        independent). This is a direct empirical \emph{counter-observation} to the
        thesis that coupling is universally beneficial.
  \item On alternating concentric rings (5 seeds), coupled HAML reaches
        $71.20\% \pm 6.43$ vs.\ $67.30\% \pm 4.37$ for the independent variant. The
        $+3.90$~pts mean difference is \emph{not statistically significant} at $n=5$,
        though the qualitative shift to ring-aligned decision boundaries is robust
        across all five seeds.
  \item On noisy moons (\textbf{single seed}), coupling helps at intermediate noise
        ($+6.27$~pts at noise $0.20$). Multi-seed validation pending.
  \item On Fashion-MNIST (784D, 10 classes, single seed, 10 epochs), test accuracy
        reaches $80.1\%$, \emph{below standard neural baselines}. The result
        demonstrates stable training in 784D (enabled by~I1), not competitive accuracy.
  \item A controlled internal reimplementation of SA-nODE is dominated by HAML across
        noise levels (single seed; not using the authors' official code).
\end{itemize}

\textbf{What the empirical evidence actually supports.}
(i)~Condition~I1 is necessary and sufficient for high-dimensional training. (ii)~The
framework trains stably to 784D with the three-phase schedule. (iii)~Bidirectional
coupling produces a qualitative geometric advantage on structurally non-convex problems
(rings), but its quantitative accuracy benefit is regime-dependent and is
\emph{negligible} on near-convex problems (MNIST). (iv)~The combinatorial novelty of
the architecture is preserved; the foundational novelty of any individual building
block is not.

\textbf{Status of this work.} This is an exploratory preprint from an independent
researcher. The code and configuration files needed to reproduce every reported number
are open-source. We welcome external replication, criticism, and refutation.

\textbf{Positioning.} HAML sits at the intersection of supervised attractor-based
classification, hierarchical Predictive Coding, hierarchical associative memories, and
implicit-depth/equilibrium models. We claim no individual-component novelty; the
contribution is the specific instantiation. Detailed positioning is provided in
...
\textbf{Hierarchy of contributions.} The paper has two contributions we consider
\emph{robust}:
\begin{enumerate}
  \item \textbf{Architectural} (Sections~\ref{sec:architecture}--\ref{sec:learning}):
        the specific combination of hierarchical PCA-decomposed subspaces,
        gradient-learned continuous Gaussian attractors, and bidirectional Predictive-
        Coding-style coupling, together with the three-phase progressive training
        schedule. The individual building blocks have direct antecedents
        (Section~\ref{sec:related}); the assembly is what we contribute.
  \item \textbf{Practical (Condition~I1)} (Section~\ref{sec:i1}): a dimension-scaled
        kernel initialisation that is empirically \emph{necessary} for the framework
        to function beyond $\sim$20 dimensions. Without~I1, Gaussian kernels saturate
        and gradients vanish.
\end{enumerate}

We additionally present \emph{theoretical observations} (Section~\ref{sec:theory})
which we make four important admissions about:
\begin{enumerate}
  \item Proposition~\ref{prop:convergence} (LaSalle dissipative convergence) is a
        \emph{standard} property of damped gradient flows on energies bounded below;
        in our framework, it serves to justify the ODE stopping criterion, not as a
        deep result.
  \item Proposition~\ref{prop:dimindep} (dimension-independent local rate) is in
        explicit \emph{tension} with our Condition~I1: with $\sigma^2 \propto d_l$,
        the scalar $\kappa^{(l)} = w/\sigma^2$ scales as $1/d_l$. The result
        characterises a property of the linearised regime around a fixed point, not a
        global dimension-independence guarantee.
  \item Proposition~\ref{prop:expressivity} (exponential expressivity gain in
        attractor count) is a worst-case representational statement; all our
        experiments use $L \in \{2, 3\}$, so the exponential factor is not exercised.
  \item Theorem~\ref{thm:map} (MAP equivalence with hierarchical PC) requires
        $\lambda = 0$, i.e.\ \emph{disabling} the repulsive component that is
        otherwise central to HAML. In the operational regime ($\lambda > 0$), HAML
        implements a PC--inference / contrastive-discriminative hybrid, not exact
        MAP inference.
\end{enumerate}

\textbf{What this paper does NOT claim.} (i)~State-of-the-art results on any benchmark.
(ii)~A universal empirical advantage of bidirectional coupling: the evidence is
\emph{mixed}, including a direct counter-observation on MNIST where the coupling
provides no measurable gain. (iii)~Superiority over SA-nODE based on the authors'
official code (we use a controlled internal reimplementation). (iv)~Multi-seed
certification on real high-dimensional data. (v)~Foundational theoretical novelty:
the theoretical components are specialisations of established results, not new
results in themselves.

\textbf{What we hope this paper offers.} A coherent, fully reproducible exploration of
a specific architectural assembly at the intersection of four mature research strands,
with an honest accounting of which components empirically pull their weight (the
hierarchical PCA decomposition and Condition~I1 do; the bidirectional coupling does so
only in some regimes), and a starting point for further investigation by others.
...
Potential               & Fixed double-well              & Parametric Gaussian mixture (with repulsion) \\
Structure               & Single dynamic level           & $L$ bidirectionally coupled levels \\
Coupling                & None                           & Bottom-up + top-down \\
High-dim.\ training     & Dimension-dependent saturation & Stabilised by Condition~I1 (Sec.~\ref{sec:i1}) \\
Theoretical anchor      & Statistical mechanics          & Dyn.\ systems + Predictive Coding \\
PC/MAP correspondence   & Not established                & Holds for $\lambda{=}0$ subset (Thm.~\ref{thm:map}) \\
\bottomrule
\end{tabular}
\caption{Architectural comparison with SA-nODE~\cite{marino2024}. We do not claim
``dimension-independent convergence'' in this row, in line with the caveats on
Proposition~\ref{prop:dimindep} (see Section~\ref{sec:theory:dimindep}).}
\label{tab:comparison_sanode}
\end{table}

\subsection{Contributions}

The contributions of this paper, in decreasing order of robustness:

\begin{enumerate}
  \item \textbf{Architectural assembly} (Section~\ref{sec:architecture}): the specific
        combination of (a)~PCA-decomposed hierarchical state space, (b)~gradient-learned
        continuous Gaussian attractors with explicit repulsion at every level, and
        (c)~bidirectional PC-style inter-level coupling, trained end-to-end with a
        three-phase progressive schedule. The combination is not present in the prior
        work we reviewed.
  \item \textbf{Condition~I1 (dimension-scaled kernel initialisation)}
        (Section~\ref{sec:i1}): an empirically \emph{necessary} initialisation rule
        $\sigma_{\mathrm{init}} \propto \sqrt{d_l}$ that prevents Gaussian-kernel
        saturation in high dimension. Without~I1, gradient magnitudes vanish and HAML
        cannot train beyond $\sim$20 dimensions (Table~\ref{tab:kernelfix}). This is
        the contribution that most directly enables the framework to function on
        real high-dimensional data.
  \item \textbf{Practical training mechanisms}: the three-phase progressive coupling
        schedule (Section~\ref{sec:three-phase}) and the heuristic level-aware
        collapse-recovery mechanism (Section~\ref{sec:p6}).
  \item \textbf{Preliminary empirical study}: 6 reproducible campaigns across
        synthetic and real datasets, with explicit reporting of single-seed limitations
        and missing baselines.
  \item \textbf{Theoretical observations} (Section~\ref{sec:theory}): four results
        --- dissipative convergence, local rate at fixed points, hierarchical
        expressivity, MAP correspondence --- presented with the caveats stated in
        Section~\ref{sec:scope} and explicit relations to prior work.
...
\subsection*{Preamble: status of the theoretical results}

Before presenting the results, we restate the epistemic position laid out in
Section~\ref{sec:scope}. Each of the four theoretical observations below
\emph{specialises} an established framework to HAML's architecture; none constitutes a
new theoretical result in its own right. We provide them because they clarify how
HAML's design choices relate to standard tools (LaSalle stability, Hessian analysis,
topological complexity, hierarchical PC inference), and because the conditions under
which each holds reveal genuine structural features of the framework. They should not
be read as foundational contributions.

The four observations are summarised in Table~\ref{tab:theory_caveats} with their
caveats made explicit. We recommend the reader consult this table while reading the
...
\begin{tabular}{p{2.6cm} p{4.8cm} p{5.6cm}}
\toprule
\textbf{Result} & \textbf{Substance} & \textbf{Caveat} \\
\midrule
Prop.~\ref{prop:convergence} (LaSalle) & Damped gradient flow on bounded-below energy converges to critical points & Standard property of \emph{any} such flow; serves as well-posedness of the ODE stopping criterion. \\
\addlinespace
Prop.~\ref{prop:correct_basin} (C1--C3) & Correct-basin convergence under (C1)~initial dominance, (C2)~sufficient coupling, (C3)~separation & Conditional and partly tautological; C2 is set by grid search, not verified analytically. \\
\addlinespace
Prop.~\ref{prop:dimindep} (local rate) & Intra-level Hessian at a fixed point is $\kappa^{(l)}\mathbf{I}_{d_l}$, scalar in $d_l$ & In \emph{tension} with Condition~I1: $\sigma^2 \propto d_l$ implies $\kappa \propto 1/d_l$. The result describes the linearised regime only. \\
\addlinespace
Prop.~\ref{prop:expressivity} (expressivity) & $O(M^L)$ representable configurations with $O(ML)$ parameters & Worst-case representational; all experiments use $L \in \{2,3\}$, so the $M^L$ factor is not empirically exercised. \\
\addlinespace
Prop.~\ref{prop:rademacher} (Rademacher) & Generalisation bound with sub-linear depth dependence & Specialisation of existing neural-ODE bounds~\cite{hanson2024rademacher,marion2023generalization}; not used operationally. \\
\addlinespace
Thm.~\ref{thm:map} (MAP correspondence) & Energy equals hierarchical PC free energy under M1--M3 & M3 requires $\lambda = 0$, which \emph{disables} the repulsive terms central to HAML's full model. The operational regime ($\lambda > 0$) is a PC + contrastive hybrid, not exact MAP. \\
\bottomrule
\end{tabular}
\caption{Theoretical results and their caveats. The detailed statements and remarks
follow in this section.}
\label{tab:theory_caveats}
\end{table}
...
\begin{remark}[Honest framing of Proposition~\ref{prop:convergence}]
This proposition is a standard property of damped gradient flows on energies bounded
below; it is not specific to HAML. We state it because the integration code relies on
it (the ODE solver terminates on the criterion $\|\dot{\mathbf{x}}^{(l)}\| < \epsilon$,
which is well-defined precisely because of dissipative convergence). It does
\emph{not} guarantee convergence to a minimum, let alone to the correct-class minimum.
Proposition~\ref{prop:correct_basin} below formalises sufficient conditions for the
latter, and is, we acknowledge, largely tautological in flavour.
\end{remark}
...
\subsection{Local Hessian Structure at a Fixed Point}
\label{sec:theory:dimindep}

\begin{proposition}[Isotropic Local Hessian at a Fixed Point]
\label{prop:dimindep}
...
\]
is a scalar parameterising the local relaxation rate $T \sim \gamma/\kappa^{(l)}$. The
Hessian is isotropic across the $d_l$ coordinates of the level.
\end{proposition}

\begin{remark}[Tension with Condition~I1, and what the result actually says]
\label{rem:dimindep_tension}
Earlier drafts of this preprint described this result as showing
\emph{dimension-independent convergence}. We retract that framing. The Hessian shape is
isotropic in $d_l$, but its scalar magnitude $\kappa^{(l)} = w/\sigma^2$ depends
critically on how $\sigma$ is set, and the two natural regimes have opposite issues:

\begin{itemize}
  \item \emph{Fixed $\sigma$} (no dimension scaling): $\kappa^{(l)}$ is independent
        of $d_l$, but the kernels saturate toward zero away from the fixed point
        (Table~\ref{tab:kernelfix}), so the dynamics never enter the linear regime:
        HAML fails to train at $d_l \gtrsim 20$.
  \item \emph{Condition~I1 active} ($\sigma^2 \propto d_l$): kernels stay active
        globally and gradients are well-conditioned, but $\kappa^{(l)} \propto 1/d_l$,
        so the local relaxation time scales as $T \sim \gamma d_l / w$, growing
        \emph{linearly} in $d_l$.
\end{itemize}

The genuine content of Proposition~\ref{prop:dimindep} is therefore: the Hessian's
isotropy means the linearised dynamics decouple uniformly across coordinates of a
level, which is a structural property of Gaussian potentials absent from polynomial
double-wells (SA-nODE). It is \emph{not} a global ``dimension-independence''
guarantee. Condition~I1 trades a $d_l$ inflation of the local relaxation time against
the ability to train at all in high dimension; this trade-off is what makes HAML
operational at 784D, not a free lunch.
\end{remark}

\begin{corollary}[Depth and local rate]
\label{cor:depth}
Under moderate coupling ($\alpha_{\mathrm{td}}\max_l\|\Pi_l\|_2$ bounded uniformly in
$L$), $\lambda_{\min}(\mathbf{H})$ of the coupled system remains bounded below by a
quantity independent of~$L$ in the linear regime. Adding levels does not, by itself,
slow local relaxation; the global convergence time and the dimension scaling through
I1 are separate questions.
\end{corollary}
...
\begin{remark}[Caveats, including empirical scope]
...
(hierarchically factorisable boundaries). It does not say that hierarchical models
always beat flat ones --- on problems without such factorisation, the gain may be zero.

A second caveat concerns the empirical scope of this paper. All our experiments use
$L \in \{2, 3\}$ (Sections~\ref{sec:emp:rings}--\ref{sec:emp:fashion}), so the
exponential factor $M^L$ is \emph{never empirically exercised} at scales where
``exponential'' is qualitatively distinguishable from ``quadratic''. The
representational gain we report is consistent with the proposition but does not
demonstrate it. Validating the $O(M^L)$ scaling experimentally would require campaigns
with $L \ge 4$ and comparison against $L=1$ baselines on problems of controlled
topological complexity. We list this in Future Work.

A third caveat is on the decision rule. HAML's prediction (Eq.~\ref{eq:predict}) is a
linear combination of per-class scores summed across levels, not a combinatorial
function of the $M^L$ joint configurations. The architecture \emph{represents} $O(M^L)$
configurations in principle; the prediction rule does not exploit them combinatorially.
A read-out that did exploit them is a possible extension.

The concentric-rings problem (Section~\ref{sec:emp:rings}) is a constructive example
where the hierarchical factorisation holds: radial structure factorises out at
level~1, angular refinement remains at level~0.
\end{remark}
...
\begin{remark}[Open empirical question: is bidirectional coupling necessary?]
\label{prop:bidi_necessary}
The original version of this preprint stated this as a conjecture: that there exist
classification problems representable by a hierarchical bidirectional system that are
not representable by independent levels with the same attractor count. We now demote it
to an \emph{open empirical question}, because our own experiments are mixed:

\begin{itemize}
  \item On the concentric-rings benchmark (Section~\ref{sec:emp:rings}), the coupled
        variant produces qualitatively different (ring-aligned, non-convex) decision
        boundaries that the independent variant does not, suggesting the coupling
        gives access to a representational regime independent levels do not reach.
  \item On the MNIST subset (Section~\ref{sec:emp:mnist}), the coupled and
        independent variants achieve \emph{identical} test accuracy ($75.8\%$ vs.\
        $75.8\%$). This is a direct counter-observation: at least for this problem
        and this protocol, bidirectional coupling does not contribute representational
        power that independent levels lack.
\end{itemize}

The honest reading is that bidirectional coupling appears to matter for problems with
genuine global-to-local hierarchical structure (rings) but not for problems that can be
solved by local feature aggregation (MNIST at this scale). Establishing the boundary
between these regimes empirically, and proving the corresponding separation result
formally, is left to future work.
\end{remark}
...
\begin{remark}[Critical caveat: M3 is \emph{not} satisfied in HAML's operational regime]
\label{rem:m3_not_satisfied}
The MAP equivalence requires condition M3 ($\lambda = 0$), i.e.\ \emph{disabling the
repulsive terms} in Eq.~\ref{eq:energy}. The repulsive terms are essential to HAML's
basin-separation mechanism (Section~\ref{sec:learning}, $\mathcal{L}_{\mathrm{sep}}$);
disabling them removes a component that the experimental code uses throughout
($\lambda > 0$ in all reported runs). The implication is uncomfortable but should be
stated openly:
\begin{itemize}
  \item \textbf{In the regime where Theorem~\ref{thm:map} holds exactly}
        ($\lambda = 0$), HAML reduces to hierarchical Predictive Coding inference
        with a Gaussian-mixture prior. This is a known construction; HAML adds
        nothing foundational here.
  \item \textbf{In the regime where HAML actually operates} ($\lambda > 0$),
        Theorem~\ref{thm:map} does not apply exactly. The energy is the sum of a PC
        free-energy term and a contrastive discriminative
        prior~\cite{larochelle2008}, and HAML implements a PC + contrastive hybrid,
        not exact MAP inference.
\end{itemize}
We retain the theorem because the decomposition
$\mathcal{E} = \mathcal{F}_{\mathrm{PC}} + \lambda\,\mathcal{F}_{\mathrm{discrim}}$
is a useful structural observation about HAML's energy, and because it suggests the
natural VAE-style extension described in Section~\ref{sec:future}. But we no longer
claim the MAP correspondence as a foundational contribution of this work: it is a
specialisation of established results to a reduced version of HAML, and the full HAML
is a known type of hybrid in the energy-based modelling literature.
\end{remark}
...
specific structural choices, and that M3 in particular excludes HAML's full operational
regime (see Remark~\ref{rem:m3_not_satisfied} above).
\end{remark}
...
\begin{enumerate}
  ...
  \item \textbf{Unexplained asymmetry}: SA-nODE-reimpl.\ degrades more slowly than HAML
        ...
        explanation.
  \item \textbf{Open empirical question on bidirectional coupling}:
        Remark~\ref{prop:bidi_necessary} now records that bidirectional coupling
        appears beneficial on rings but not on MNIST. Establishing the structural
        boundary between these regimes is open.
...
\begin{enumerate}
  ...
  \item \textbf{Resolution of Remark~\ref{prop:bidi_necessary}}: structural
        characterisation of problems where bidirectional coupling actually adds
...
We introduced HAML, a supervised classification framework that combines hierarchical
PCA-decomposed state spaces, gradient-learned continuous Gaussian attractors, and
bidirectional Predictive-Coding-style coupling. The contributions, in decreasing order
of robustness, are: (i)~an original architectural assembly of these primitives;
(ii)~Condition~I1, a dimension-scaled kernel initialisation that is empirically
necessary for high-dimensional training; (iii)~a practical three-phase progressive
training schedule and a heuristic collapse-recovery mechanism; and (iv)~a preliminary
empirical study with explicit single-seed limitations.

The four theoretical observations in Section~\ref{sec:theory} (dissipative convergence,
isotropic local Hessian, hierarchical expressivity, PC--MAP correspondence) are
\emph{specialisations} of established results to HAML's specific architecture, with
caveats stated explicitly in Section~\ref{sec:scope} and Table~\ref{tab:theory_caveats}.
They are not foundational theoretical contributions. In particular,
Proposition~\ref{prop:dimindep} stands in tension with Condition~I1 (the trade-off is
laid out in Remark~\ref{rem:dimindep_tension}), and Theorem~\ref{thm:map} applies
exactly only when the repulsive component of HAML is disabled
(Remark~\ref{rem:m3_not_satisfied}).

Empirically, the evidence is preliminary and mixed. The framework trains stably in 784D
($80.1\%$ Fashion-MNIST, single seed, below standard baselines). Bidirectional coupling
provides a qualitative geometric advantage on structurally non-convex problems (rings)
and a single-seed accuracy gain at intermediate noise (moons), but \emph{no measurable
gain} on near-convex problems (MNIST subset). The latter is a direct empirical
counter-observation to the thesis that hierarchical coupling is universally beneficial,
and we report it explicitly rather than hide it. Multi-seed validation and comparison
against strong neural baselines remain the most important next steps.

We release the framework and all experimental code in the hope that it will be useful
as a starting point for further investigation of attractor-based classification,
hierarchical Predictive Coding implementations, and energy-based hybrids. We welcome
external replication, criticism, and refutation.
...
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{2.3cm}lp{4cm}p{4.3cm}}
\toprule
\textbf{Result} & \textbf{Section} & \textbf{Substance} & \textbf{Caveat / status} \\
\midrule
Prop.~\ref{prop:convergence} (LaSalle) & \ref{sec:theory:convergence} & Damped gradient flow converges to critical points. & Standard for any damped gradient flow on bounded energy. \\
\addlinespace
Prop.~\ref{prop:correct_basin} (C1--C3) & \ref{sec:theory:convergence} & Correct-basin convergence under explicit conditions. & C1--C3 are not verified analytically; partly tautological. \\
\addlinespace
Prop.~\ref{prop:guidance} (Hierarchical guide) & \ref{sec:theory:convergence} & Coupling shifts Hessian eigenvalues positively. & Standard Weyl perturbation argument. \\
\addlinespace
Prop.~\ref{prop:dimindep} (Isotropic Hessian) & \ref{sec:theory:dimindep} & Local Hessian at fixed point is $\kappa^{(l)}\mathbf{I}_{d_l}$. & In \emph{tension} with~I1: $\kappa \propto 1/d_l$ with I1. Not a global dim-independence. \\
\addlinespace
Prop.~\ref{prop:expressivity} (Expressivity) & \ref{sec:theory:expressivity} & $O(M^L)$ configurations with $O(ML)$ parameters. & Worst-case; $L=2,3$ in all experiments. \\
\addlinespace
Rem.~\ref{prop:bidi_necessary} (Bidi.\ open Q) & \ref{sec:theory:expressivity} & Bidirectional coupling helps on rings, neutral on MNIST. & Empirically open. \\
\addlinespace
Prop.~\ref{prop:rademacher} (Rademacher) & \ref{sec:theory:expressivity} & Sub-linear-in-depth complexity bound. & Specialisation of existing neural-ODE bounds; not used operationally. \\
\addlinespace
Thm.~\ref{thm:map} (MAP) & \ref{sec:theory:bayes} & $\mathcal{E} = \mathcal{F}_{\mathrm{PC}}$ under M1--M3. & M3 ($\lambda=0$) excludes HAML's operational regime. Specialisation of known correspondences. \\
\addlinespace
\midrule
\multicolumn{4}{l}{\emph{Non-theoretical contributions (the more robust ones):}} \\
\addlinespace
Cond.~I1 & \ref{sec:i1} & $\sigma_{\mathrm{init}} \propto \sqrt{d_l}$ initialisation. & Empirically necessary for $d > 20$. \\
\addlinespace
Prop.~\ref{prop:p6} (Level-aware recovery) & \ref{sec:p6} & Heuristic to re-anchor collapsed levels. & Engineering, not a theorem. \\
\bottomrule
\end{tabular}
\caption{Summary of theoretical results, with their epistemic status. The genuinely
robust contributions of the paper (architectural assembly, Condition~I1) are listed at
